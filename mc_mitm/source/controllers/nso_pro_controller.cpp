 ...
 */
#include "nso_pro_controller.hpp"
...
namespace ams::controller {
    ...
    NSOProController::NSOProController(const bluetooth::Address *address, HardwareID id)
    ...
    Result NSOProController::HandleDataReportEvent(const bluetooth::HidReportEventInfo *event_info) {
                    ...
                    if (response.data.serial_flash_read.address == 0x6050) {
                        ...
                        switch (response.data.serial_flash_read.address) {
                            case 0x603d: //Factory stick calibration
                                ...
                                memcpy(&m_n64_calibrated_zero, &response.data.serial_flash_read.data[3], 3);
                            ...
                            case 0x8010: //User stick calibration
                                if (response.data.serial_flash_read.data[0] != 0xFF && response.data.serial_flash_read.data[1] != 0xFF) {
                                    ...
                                    memcpy(&m_n64_calibrated_zero, &response.data.serial_flash_read.data[5], 3);
        ...
        if (m_is_n64_controller) {
            if (ApplyN64Remapping(&input_report->buttons)) {
                ...
                memcpy(&input_report->left_stick.m_xy, &m_n64_calibrated_zero, 3);
            }
            else memcpy(&input_report->right_stick.m_xy, &m_n64_calibrated_zero, 3);
        }
    ...
    Result NSOProController::HandleOutputDataReport(const bluetooth::HidReport *report) {
