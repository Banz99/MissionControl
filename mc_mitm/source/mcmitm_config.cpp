...
namespace ams::mitm {
        ...
        int MissionControlConfigIniHandler(void *user, const char *section, const char *name, const char *value) {
            ...
            if (strcasecmp(section, "bluetooth") == 0) {
                if (strcasecmp(name, "host_name") == 0)
                    ...
                    io_conversions::ParseBluetoothAddress(value, &config->bluetooth.host_address);
            }
            else if (strcasecmp(section, "general") == 0) {
                if (strcasecmp(name, "disable_custom_profiles") == 0)
                    io_conversions::ParseBoolean(value, &config->general.disable_custom_profiles);
            }
            else {
                return 0;
            }
    ...
    void ParseIniConfig(void) {
        ...
        /* Parse the config. */
        util::ini::ParseFile(file, &g_global_config, MissionControlConfigIniHandler);

        profiles::SetProfilesDisabled(g_global_config.general.disable_custom_profiles);
    }
